function activateDrag(e) {
  ...
  Array.prototype.map.call(voting_list, (list) => {draggableList(list)});
}

function draggableList(listElement) {
  Array.prototype.map.call(listElement.children, (element) => {startDrag(element)});
}
...
//Pop-up when trying to click on the submit button. If visitor confirms submssion, the page will be reloades, resetting the voting page.
function clicked_button() {
  if (confirm('Are you sure you want to submit? When submitting, your answers will be sent and the voting page will be reset')) {
