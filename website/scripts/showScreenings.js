...
function generateAnimeScreenings (animeList, includeEpisodes) {
  ...
  for (series in animeList) { //Iterates through the series in animeList
    ...
    if (!includeEpisodes) { //Generate div with no episodes
      animeScreenings += addEpisodes(anime, imgsrc, title, 0, 0)
    } else if (episode < 0) { //Generate no div, increment anime
      episodeIncrement(anime, 2)
    } else if (episode == 0) { //Generate div with first episode of anime
      animeScreenings += addEpisodes(anime, imgsrc, title, episode + 1, 1)
      episodeIncrement(anime, 1)
    } else if (episode < totalEpisodes) { //Generate div with two episodes of anime
      animeScreenings += addEpisodes(anime, imgsrc, title, episode, 2)
    } else if (episode == totalEpisodes) { //Generate div with last episode of anime
